...
namespace AFBase
{
	...
	const string g_afInfo = "AFBase 1.3.2 PUBLIC modified";
...
class AFBaseBase : AFBaseClass
{
	...
	void ExpansionInit()
	{
		...
		RegisterCommand("afb_who", "!b", "<0/1 don't shorten nicks> - Show client information", ACCESS_D, @AFBaseBase::who);
...
namespace AFBaseBase
{
	...
	void ungag(AFBaseArguments@ AFArgs)
	{
		...
		if(AFBase::GetTargetPlayers(AFArgs.User, HUD_PRINTCONSOLE, AFArgs.GetString(0), TARGETS_NOALL|TARGETS_NOAIM|TARGETS_NORANDOM, pTargets))
		{
			...
			for(uint i = 0; i < pTargets.length(); i++)
			{
				...
				afbasebase.TellAll("An Admin ungagged player \""+AFArgs.GetString(0)+"\"", HUD_PRINTTALK);
	...
	void gag(AFBaseArguments@ AFArgs)
	{
		...
		if(AFBase::GetTargetPlayers(AFArgs.User, HUD_PRINTCONSOLE, AFArgs.GetString(0), TARGETS_NOALL|TARGETS_NOAIM|TARGETS_NORANDOM, pTargets))
		{
			...
			for(uint i = 0; i < pTargets.length(); i++)
			{
				...
				afbasebase.TellAll("An Admin gagged player \""+AFArgs.GetString(0)+"\" (mode: "+sOutMode+")", HUD_PRINTTALK);
	...
	void bandecals(AFBaseArguments@ AFArgs)
	{
		...
		if(AFBase::GetTargetPlayers(AFArgs.User, HUD_PRINTCONSOLE, AFArgs.GetString(0), TARGETS_NOALL|TARGETS_NOAIM|TARGETS_NORANDOM, pTargets))
		{
			...
			for(uint i = 0; i < pTargets.length(); i++)
			{
				...
				if(bMode)
				{
					afbasebase.TellAll("An Admin banned player \""+AFArgs.GetString(0)+"\" from spraying decals", HUD_PRINTTALK);
					...
					afbasebase.TellAll("An Admin unbanned player \""+AFArgs.GetString(0)+"\" from spraying decals", HUD_PRINTTALK);
	...
	void banlate(AFBaseArguments@ AFArgs)
	{
		...
		if(sHold.SubString(0,6).ToLowercase() == "steam_")
		{
			if(iMinutes > 0)
			{
				afbasebase.TellAll("An Admin banned \""+AFArgs.GetString(0)+"\" for "+string(iMinutes)+" minutes", HUD_PRINTTALK);
				...
				afbasebase.TellAll("An Admin banned \""+AFArgs.GetString(0)+"\" permanently", HUD_PRINTTALK);
		...
		if(AFBase::GetTargetPlayers(AFArgs.User, HUD_PRINTCONSOLE, AFArgs.GetString(0), TARGETS_NOALL|TARGETS_NOAIM|TARGETS_NORANDOM, pTargets))
		{
			...
			for(uint i = 0; i < pTargets.length(); i++)
			{
				...
				if(sHold != "")
				{
					...
					if(iMinutes > 0)
					{
						afbasebase.TellAll("An Admin banned player "+pTarget.pev.netname+" ("+sFill+") for "+string(iMinutes)+" minutes (reason: "+sReason+")", HUD_PRINTTALK);
						...
						afbasebase.TellAll("An Admin banned player "+pTarget.pev.netname+" ("+sFill+") permanently (reason: "+sReason+")", HUD_PRINTTALK);
	...
	void unban(AFBaseArguments@ AFArgs)
	{
		...
		if(sHold.SubString(0,6).ToLowercase() == "steam_")
		{
			...
			{
				afbasebase.TellAll("An Admin unbanned "+sHold, HUD_PRINTTALK);
	...
	void slap(AFBaseArguments@ AFArgs)
	{
		...
		if(AFBase::GetTargetPlayers(AFArgs.User, HUD_PRINTCONSOLE, AFArgs.GetString(0), 0, pTargets))
		{
			...
			{
				@pTarget = pTargets[i];
				afbasebase.TellAll("An Admin slapped player "+pTarget.pev.netname+" with "+string(iDamage)+" damage", HUD_PRINTTALK);
				afbasebase.Tell("Slapped player "+pTarget.pev.netname+" with "+string(iDamage)+" damage", AFArgs.User, HUD_PRINTCONSOLE);
				afbasebase.Log("Admin "+AFArgs.User.pev.netname+" slapped player "+pTarget.pev.netname+" with "+string(iDamage)+" damage");
	...
	void slay(AFBaseArguments@ AFArgs)
	{
		...
		if(AFBase::GetTargetPlayers(AFArgs.User, HUD_PRINTCONSOLE, AFArgs.GetString(0), 0, pTargets))
		{
			...
			{
				@pTarget = pTargets[i];
				afbasebase.TellAll("An Admin slayed player "+pTarget.pev.netname, HUD_PRINTTALK);
				...
				entvars_t@ world = g_EntityFuncs.Instance(0).pev;
				pTarget.TakeDamage(world, world, 65536.0f, DMG_ALWAYSGIB|DMG_CRUSH);
				if(AFBase::IsSafe())
				{
					TraceResult tr;
	...
	void changelevel(AFBaseArguments@ AFArgs)
	{
		...
		afbasebase.TellAll("An Admin changed level to "+sMap, HUD_PRINTTALK);
	...
	void kick(AFBaseArguments@ AFArgs)
	{
		...
		if(AFBase::GetTargetPlayers(AFArgs.User, HUD_PRINTCONSOLE, AFArgs.GetString(0), TARGETS_NOALL|TARGETS_NOAIM|TARGETS_NORANDOM, pTargets))
		{
			...
			{
				@pTarget = pTargets[i];
				afbasebase.TellAll("An Admin kicked player "+pTarget.pev.netname+" (reason: "+sReason+")", HUD_PRINTTALK);
		...
		if(AFBase::GetTargetPlayers(AFArgs.User, HUD_PRINTCONSOLE, AFArgs.GetString(0), TARGETS_NOIMMUNITYCHECK, pTargets))
		{
			...
			for(uint i = 0; i < pTargets.length(); i++)
			{
				...
				if(sFlags.SubString(0,1) == "+")
				{
					...
					afbasebase.TellAll("An Admin updated "+pTarget.pev.netname+" access to \""+sNewAccess+"z\"", HUD_PRINTTALK);
				...
				}else if(sFlags != "!")
				{
					...
					afbasebase.TellAll("An Admin updated "+pTarget.pev.netname+" access to \""+sNewAcc+"\"", HUD_PRINTTALK);
